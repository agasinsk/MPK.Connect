...
export class StopMap extends Component {
  ...
  render() {
    ...
    if (this.state.timeTableOpened) {
      timeTable = <TimeTable open={this.state.timeTableOpened} onClose={this.toggleTimeTable} timeTable={this.state.timeTable}></TimeTable>
    }
      ...
      <React.Fragment>
        ...
        <Map ref={m => { this.leafletMap = m; }} center={mapCenter} zoom={zoomLevel}>
          ...
          {this.state.visibleStops.map((stop) => {
            ...
            return <Marker key={`marker-${stop.id}`} position={position} onClick={() => this.getTimeTable(stop.id)}>
              <Popup>
                <span>{stop.name}
                  <br /> {stop.latitude}
                  <br /> {stop.longitude}
